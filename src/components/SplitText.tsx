...
const SplitText = ({
  ...
  useGSAP(
    () => {
      ...
      // If animation is disabled or already completed, set final state immediately
      if (!shouldAnimate || hasAnimatedRef.current) {
        // Set all characters to final state immediately
        ...
        if (chars.length > 0) {
          ...
          // If no chars yet, set the element itself
